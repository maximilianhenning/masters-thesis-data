...
voyage_file_list = glob(path.join(dir, "..", "processing", "voyages_text/*"))
overall_list = []
# Apply function
for file in voyage_file_list:
    ...
    # Save files
    print("Cleaned", file_title, "- Original lines:", str(line_counter) + ", Cleaned lines:", str(line_cleaned_counter))
    if not path.exists(path.join(dir, "..", "processing", "voyages_clean")):
        makedirs(path.join(dir, "..", "processing", "voyages_clean"))
    with open(path.join(dir, "..", "processing", "voyages_clean", file_title + ".txt"), "w") as output:
        for line in lines_cleaned:
            output.write(line + "\n")
...
officer_folder_list = glob(path.join(dir, "..", "processing", "officers_png/*"))
overall_list = []
if not path.exists(path.join(dir, "..", "processing", "officers_text")):
    makedirs(path.join(dir, "..", "processing", "officers_text"))
# OCR files
for folder in officer_folder_list: 
    folder_title = folder.split("\\")[-1].split(".")[0]
    if not path.exists(path.join(dir, "..", "processing", "officers_text", folder_title + ".txt")):
        files = glob(folder + "/*")
        folder_ocr = []
        for file in files:
            file_ocr = pytesseract.image_to_string(Image.open(file))
            folder_ocr += file_ocr
        with open(path.join(dir, "..", "processing", "officers_text", folder_title + ".txt"), "w") as output:
            for char in folder_ocr:
                output.write(char)
    # Monitoring
    print("OCR'd", folder_title)
# Clean files
ocr_file_list = glob(path.join(dir, "..", "processing", "officers_text/*"))
if not path.exists(path.join(dir, "..", "processing", "officers_clean")):
    makedirs(path.join(dir, "..", "processing", "officers_clean"))
for file in ocr_file_list: 
    ...
    # Save files
    with open(path.join(dir, "..", "processing", "officers_clean", file_title + ".txt"), "w") as output:
        for line in file_lines:
            output.write(line + "\n")
    # Monitoring
    print("Cleaned", file_title)
# Save combined file for debugging
